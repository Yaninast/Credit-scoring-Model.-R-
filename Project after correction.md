...
```{r}
# Define p?th and file;

setwd('C:\\Users\\yanin\\Desktop\\Capstone\\Project Data')
...
stargazer(tra?nt_test_val, type=c('html'),out=paste(out.path,file.name,sep=''),
...
#1.Remap i?valid observations of EDUCATION variable to valid data points
```{r}
...
#We don't known what the respondents meant?by placing 5 and 6 values as their answers we mi predict the values using multiple imputation 
...
data_subset$EDUCATION[?ata_subset$EDUCATION == 5 | data_subset$EDUCATION == 6 | data_subset$EDUCATION == 0] <- NA
summary(data_subset)
...
credit_card_defa?lt$EDUCATION<-imputedData$EDUCATION
...
```
#2. Remap invalid observati?ns of MARRIAGE variable to valid values

```{r}
table(credit_card_default$MARRIAGE) # the range of the variable' values does not align with the one expected based on the dictionary

# replace the typo or out of range values with NA and predict the missing ?alues using multiple imputation and cart method 
...
# Impute NA's using cart met?ods within MICE function 
...
# check and compare the distribut?on of the imputed and original data 
...
table(credit_card_defau?t$MARRIAGE)
...
```
# Need t? make sure the data is understood correctly by R, since we have a mix of numerical and categorical variables 
```{r}
...
```
# Distribution of PAY variable conditioned by credit_card_default$BILL_AMT2 == credit_card_default$PAY_?MT1
...
```
# Distribution of PAY variable conditioned by credit_card_default$BILL_AMT2 != credit_card_default$PAY_AMT1
#& credit_card_default$PAY_AMT1!= 0
```{r}
...
stargazer(r?sults_2, type=c('html'),out=paste(out.path, file.name, sep=''),title=c('Table 3. The frequency table of Pay variable under condition that the balance has been partially repaid'), align=TRUE, digits=2, initial.zero=TRUE, summary=FALSE )
```
# Explore PAY_variable 
```{r}

table(credit_card_default$BILL_AMT3 > credit_card_default$PAY_AMT2,credit_card_default$PAY_3)
table(credit_card_default$PAY_3)

credit_card_default[credit_card_default$BILL_AMT3 > credit_card_default$PAY_AMT2 & credit_card_default$PAY_3 == 1,]

table(credit_card_default$BILL_AMT2 <= credit_card_default$PAY_AMT1,credit_card_default$PAY_2)
...
table(credit_card_default$PAY_3[credit_card_default$PAY_AMT2 == 0 & credit_card_default$BILL_AMT3 !=0])

credit_card_default[credit_card_default$PAY_AMT2 != credit_card_default$BILL_AMT3 & credit_card_default$PAY_3 == -1,c(7,8,9,13,14,15,16,19,20,21,22)]


table((credit_card_default$BILL_AMT3 > credit_card_default$PAY_AMT2 & credit_card_default$PAY_AMT2> 0), credit_card_default$PAY_3)

table((credit_card_default$BILL_AMT2 > credit_card_default$PAY_AMT1 & credit_card_default$PAY_AMT1> 0), credit_card_deffult$PAY_2)
table(credit_card_default$PAY_2)
...
table(credit_card_default$PAY_3[credit_card_defa?lt$PAY_AMT2 == 0 & credit_card_default$BILL_AMT3 !=0])

credit_card_default[credit_card_default$PAY_AMT2 > credit_card_default$BILL_AMT3 & (credit_card_default$PAY_AMT2 == 0 |credit_card_default$BILL_AMT3 <0) & credit_card_default$PAY_3 == 0,c(7,8,9,13,14,?5,16,19,20,21,22)]

credit_card_default[credit_card_default$BILL_AMT3 <= credit_card_default$PAY_AMT2 & credit_card_default$PAY_3 == 0 ,c(7,8,9,13,14,15,16,19,20,21,22)]

table(credit_card_default$BILL_AMT3 > 0 & credit_card_default$BILL_AMT4 > 0 & credit_card_default$PAY_AMT3 == 0 & credit_card_default$PAY_AMT2==0, credit_card_default$PAY_2)
table(credit_card_default$PAY_3)

table(credit_card_default$BILL_AMT3 > 0 & credit_card_default$PAY_AMT2== 0 & credit_card_default$PAY_AMT1== 0, credit_card_default$PAY?3)
table(credit_card_default$PAY_3)

credit_card_default[!(credit_card_default$BILL_AMT3 > 0 & credit_card_default$PAY_AMT2== 0 & credit_card_default$PAY_AMT1== 0) & credit_card_default$PAY_3 == 3, c(7,8,9,13,14,15,16,19,20,21,22)]

credit_card_default[ credit_card_default$PAY_AMT2== 0 & credit_card_default$PAY_AMT1== 0, c(7,8,9,13,14,15,16,19,20,21,22)]

credit_card_default[credit_card_default$PAY_2 == 3 ,c(7,8,9,13,14,15,16,19,20,21,22)]

round((table(credit_card_default$PAY_2[credit_card_default$BILL_AMT2 > credit_card_default$PAY_AMT1& credit_card_default$PAY_AMT1==0])/length(credit_card_default$PAY_2[credit_card_default$BILL_AMT2 > credit_card_default$PAY_AMT1& credit_card_default$PAY_AMT1== 0])*100),2)

round((table(credit_card_default$PAY_3[credit_card_?efault$BILL_AMT3 > 0 & credit_card_default$BILL_AMT4 > 0 & credit_card_default$PAY_AMT3 == 0 & credit_card_default$PAY_AMT2==0])/length(credit_card_default$PAY_3[credit_card_default$BILL_AMT3 > 0 & credit_card_default$BILL_AMT4 > 0 &  credit_card_default$P?Y_AMT3 == 0 & credit_card_default$PAY_AMT2==0])*100),2)

table(credit_card_default$BILL_AMT3 > 0 & credit_card_default$PAY_AMT2== 0 & credit_card_default$BILL_AMT2 > credit_card_default$BILL_AMT3, credit_card_default$PAY_3)
table(credit_card_default$PAY_3)
...
```
# Map -2 values to -1 in Pay variable
```{r}
...
#  Bin Age variable using WOE binning;
#?###################################################################
...
head(credit_card_default, 10)
```
#10. Balance Growth Over 6 Months (Bal_Growth_6mo) - Is the balance growing due to continued spe?ding with only partial payments each month?
```{r}
...
```
#19. Create a REPAY_PATTERN variable to categorize the bank customers according to their balance repayment patterns 
```{r}
...
# Square root and log transformation or truncating the outliers to the 99 percentile values are considered as a remedy for right skeweness of the variable. 
...
# Another method considered to remedy an efffect of outliers is to bin the variable. 
...
#6. train$Max_Pmt_Amt
```{r}
# Use woe.Binning and Optbin to advise if th? outliers could be remapped to the respective percentiles' threshold  
...
train$Max_Pmt_Amt_tfm<- yjPower(?rain$Max_Pmt_Amt, 0, jacobian.adjusted = FALSE)
```
...
colnames(meadian_pmt_ratio)<- c("DEFAULT", 'Pmt_Ratio_2', 'Pmt_Ratio_3', 'Pmt_Ratio_?', 'Pmt_Ratio_5', 'Pmt_Ratio_6', 'Avg_Pmt_Ratio')
...
out.path = 'C:\\Users\\yanin\\Desktop\\Cap?tone\\Project Output\\'
...
credit_card_default$Avg_Bill_Amt_tfm<-yjPower(credit_card_default$A?g_Bill_Amt_tfm, 0.2, jacobian.adjusted = FALSE)
...
train_bin<- train[-c(2,7,8,21,24,25,29,30,32,34,35,47:57)]# 25 - sqrt_LIM_BAL
train_WOE <- train[-c(2,6,7,8,21,22,23,24,25,29,30,31,32,33,34,35,37,40,43,44:46)]#20 - sqrt_LIM_BAL
...
stargazer(bank_df, type=c('html'),out=paste(out.path,file.name,sep=''),
	...
	align=TRUE, digits=2, digits.extra=2, initial.zero=TRUE, summary = FALSE)
```
# Model Development. Random Forest algorithm 
```{r}
...
makeIntegerParam("nodesize", lo?er = 10, upper = 100)
)
...
rf_tune_2 <- tuneParams(learner = rf, resampling = set_cv, task = trainTask, par.set = rf_param, control = rancontrol, measures = acc, show.info = T)

par_2<- rf_tune_2$x

rf_tune_2$y

#Let's build the random forest model now a?d check its accuracy.
#using hyperparameters obtained from gridsearch nad randomserach
...
#train a model
rforest_2 <- train(rf.tree_2, trainTask)
#make predictions
rfmodel_2 <- predict(rforest_2, testTask)

calculateConfusionMatrix(rfmodel_2, relative = TRUE, sums = TRUE, set = "both")


# RandomSearch hypertuning with maximizing f1 and CV = 3
rf_tune_f1 <- tuneParams(le?rner = rf, resampling = set_cv, task = trainTask, par.set = rf_param, control = rancontrol, measures = f1, show.info = T)
...
rfmodel_f1.in_samp?e <- predict(rforest_f1, trainTask)
...
#set tunable p?rameters
#use random and grid search to find hyperparameters
rf_param <- makeParamSet(
makeIntegerParam("ntree",lower = 50, upper = 1000),
makeIntegerParam("mtry", lower = 3, upper = 33),
makeIntegerParam("nodesize", lower = 10, upper = 100)
...
```
# ROC. Precesion/Recall curve. Rfmodel
```{r}
library(ROCR)
## ROC Curve and Area under ROC
##-------------------------------------------------------------------------------

# Create prediction objec? for the training dataset
...
leg?nd (0.8, 0.4, round(auc.rfmodel,4), title = "AUC", cex = 1.0, col = "blue")
...
## KS Statistic
TPR.rfmodel <- unlist(slot(rocDat?.rfmodel, "y.values"))
...
```
#ROC. Precsion/Recall curve. Rfmodel_f1
```{r}
## ROC Curve and Area under ROC
...
```
# XGboost. Tuned using F1. Train_bin data 
```{r}
...
#def?ne search function
rancontrol <- makeTuneControlRandom(maxit = 100L,tune.threshold = TRUE) #do 100 iterations

#3 fold cross validation
set_cv <- makeResampleDesc("CV",iters = 3L)
#tune parameters
xg_tune <- tuneParams(learner = xg_set, task = trainTask, r?sampling = set_cv,measures = f1, par.set = xg_ps, control = rancontrol)
#set parameters
...
```
#Xgboost. ROC and Precsion - Recall Curve 
```{r}
## ROC Curve and Area under ROC
##-------------------------------------------------------------------------------
...
#tune para?eters to maximize TPR
...
```
#XGboost. Train_original Data 
```{r}
# XGboost. Tuned using F1. Train_original data 
...
```
#Champion Random Forest model. Feature importnace plots. 
```{r}
...
# stepwise model with interaction terms ?ncluded 
...
pred_forw?rd_2 <- predict(model_fit_forward_2, newdata=test_WOE)
...
caret::confusionMat?ix(data=pred_backward, test_WOE$DEFAULT, positive = "1")
...
```
# Logistic regression. Subjective model 
```{r}
...
summary(stepw_subj)
```
# Subjective models. Cross validation 
```{r}
...
```
# Crossvalidation. Stepwise_subjective model no interaction terms 
```{r}
...
```
#Stepwise_subjective model outsample performance
```{r}
...
stargazer(summ$coeftable, type=c('html'),out=paste(out.path,file.name,sep=''),
	...
	align=TRUE, digits=2, digits.extra=2, initial.zero=TRUE, summary = FALSE)
```
#Stepwise_subjcetive model no MARRIAGE variable. Out-of - sample performance
```{r}
...
stargazer(summ$coeftable, type=c('html'),out=paste(out.path,file.name,sep=''),
	...
	align=TRUE, digits=2, digits.extra=2, initial.zero=TRUE, summary = FALSE)
```
#Cross-validation stepwise_subjective no Marriage variable model
```{r}
# Stepwise_subjective model NO INTERACTION TERMS 
...
```
## ROC Curve and Area under ROC. Out- of - sample SAMPLE
```{r}
       # Create prediction object for the training dataset
...
stack_cv <- resample(learner = stack_learner, task = trainTask_enc, resampl?ng = rdesc, measures = list(tpr,fpr,fnr,fpr,acc), show.info = T)
...
stack_lrn.?redict<- setThreshold(stack_lrn.predict,0.1521336)

calculateConfusionMatrix(stack_lrn.predict, relative = TRUE, sums = TRUE, set = "both")
```
# Ensemble Model. ROC. Precsion - Recall curve 
```{r}
...
# Ca?culate Area Under ROC Curve
...
rf_tune_grid <- tuneParams(learner = rf, resampling = set_cv, task = trainTask, par.set = rf_param, control = rancontrol, measures = f1, show.info = T)

par_grid<- rf_tune_grid

rf_tune_grid$y

#Let's build the random forest model now and check its ac?uracy.
#using hyperparameters obtained from gridsearch nad randomserach

rf.grid.tuned <- setHyperPars(rf, par.vals = rf_tune_grid$x)

r <- resample(learner = rf.grid.tuned, task = trainTask, resampling = rdesc, measures = list(tpr,fpr,fnr,tnr,acc, f1, mlr::auc, ppv), show.info = T)

#train model
rf_grid_model <- mlr::train(rf.grid.tuned, trainTask)
#test model
predict.rf_grid <- predict(rf_grid_model, testTask)
calculateConfusionMatrix(predict.rf_grid, relative = TRUE, sums = TRUE, set = "both")
tuneThreshold(predict.rf_grid, f1)
predict.rf_grid<- setThreshold(predict.rf_grid,0.1530386)

predict_grid.in_sample <- predict(rf_grid_model, trainTask)
tuneThreshold(predict_grid.in_sample, f1)
predict_grid.in_sample <- setThreshold(predict_grid.in_sample,0.06875046)

calculateConfusionMatrix(predict_grid.in_sample, relative = TRUE, sums = TRUE, set = "both")


# Calculate Area Under ROC Curve
ROCR::performance(asROCRPrediction(predict_grid.in_sample), "auc")
rocData<- ROCR::performance (asROCRPrediction(predict_grid.in_sample), "tpr", "fpr")
...
```
# Random Forest Grid Search tuned with F1 parameter 
```{r}
## ROC Curve and Area under ROC
##-------------------------------------------------------------------------------

# Create prediction object for the training dataset
predition_rf_grid <- asROCRPrediction(predict.rf_grid)

# get data for ROC Curve
rocData.rf_grid<- ROCR::performance (predition_rf_grid, "tpr", "fpr")

# Calculate Area Under ROC Curve
auc.rf_grid<- ROCR::performance(predition_rf_grid, "auc")
auc.rf_grid<- unlist(slot(auc.rf_grid, "y.values"))
print(auc.rf_grid)

# Plot ROC curve
plot(rocData.rf_grid, colorize = T,
main = "ROC Curve. GridSearch tuned Random Forest model",
ylab = "Sensitivity (TPR)",
xlab = "1-Specificity (FPR)")
legend (0.8, 0.4, round(auc.rf_grid,4), title = "AUC", cex = 1.0, col = "blue")
# Draw line in the middle
abline(a=0, b = 1)
...
## Precision vs Recall (TPR)
precObj.rf_grid <- ROCR::performance (predition_rf_grid, "prec", "rec")
plot(precObj.rf_grid, colorize = T,
main = "Precision-Recall Curve. GridSearch tuned Random Forest model",
ylab = "Precision", xlab = "Recall (TPR)")
legend (0.2, 0.5, round(auc.rf_grid,4), title = "AUC", cex = 1.0, col = "blue")


## KS Statistic
TPR.rf_grid <- unlist(slot(rocData.rf_grid, "y.values"))
FPR.rf_grid <- unlist(slot(rocData.rf_grid, "x.values"))
diffTPR.FPR.rf_grid<- TPR.rf_grid-FPR.rf_grid
KS.rf_grid <- max(diffTPR.FPR.rf_grid)
KS.rf_grid
...
xg_set <- makeLearner("classif.xgboost", predi?t.type = "prob")
xg_set$par.vals <- list(
objective = "binary:logistic",
eval_metric = "auc",
nrounds = 250
...
tuneThreshold(predict.xg?grid, f1)
...
# Calculate Area Under ROC Curve
a?c.xg_grid<- ROCR::performance(predition_xg_grid, "auc")
...
```
#Stacking model ROC and Precion Recall 
```{r}
library(mlr)
library(ROCR)
## ROC Curve and Area under ROC
...
KS.stacking
```
# Stacking model. In - sample performance 
```{r}
...
```
# Naive Bayes. ROC and Precsion Recall 
```{r}
## ROC Curve and Area under ROC
##-------------------------------------------------------------------------------
...
```
#KS statistics with Statmeasure library 

```{r}
library(StatMeasures)
ks(test_one_hot$DEFAULT, predition_stacking)
```
...
# Check the min score in each model decile;
aggregate(my.df$pred_stepwise,by=list(Decile=my.df$model.decile),FUN=min)
